...
- **Remediation Contract:** Delegate remote image retrieval to an explicit, user-authorized background helper that enforces the mandatory dual-level acceptance contract specified in canonical specification [`docs/specs/sec005-webp-profile.md`](specs/sec005-webp-profile.md) (v1.0.0):
  ...
  8. **Local Image Asynchronous Decoding:** For local images, specify `asynchronous: true` to prevent main-thread decoding stalls on large bitmaps.
